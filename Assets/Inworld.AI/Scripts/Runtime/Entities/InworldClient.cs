...
namespace Inworld
{
	...
	internal class InworldClient
	{
		...
		internal async void GetAppAuth(string key, string secret, string sessionToken = "")
		{
#if UNITY_EDITOR
			...
			if (string.IsNullOrEmpty(sessionToken))
			{
				...
				try
				{
					m_InworldAuth.Token = await m_WorldEngineClient.GenerateTokenAsync(gtRequest, metadata, DateTime.UtcNow.AddHours(1));
					...
					(
						async () =>
						{
							while (SessionStarted)
							{
								...
								if (next)
								{
									ResolveGRPCPackets(m_StreamingCall.ResponseStream.Current);
								}
								else
								{
		...
		internal TextEvent ResolvePreviousPackets(GrpcPacket response)
		{
			return response.Text != null ? new TextEvent(response) : null;
		}

		public async void SaveGame(string workspaceFullName)
		{
			if (string.IsNullOrEmpty(SessionID))
				return;
			Channel channel = new Channel(InworldAI.Game.RuntimeServer, new SslCredentials());
			try
			{
				StateSerialization.StateSerializationClient client = new StateSerialization.StateSerializationClient(channel);
				GetSessionStateRequest request = new GetSessionStateRequest
				{
					Name = $"{workspaceFullName}/sessions/{SessionID}"
				};
				Metadata metadata = new Metadata
				{
					{
				...
				try
				{
					SessionState response = await client.GetSessionStateAsync(request, metadata).ResponseAsync;
					LastState = response.State.ToBase64();
					InworldAI.Log("===== Dialog saved =====");
					PlayerPrefs.SetString(InworldController.CurrentScene.fullName, LastState);
				}
				catch (RpcException e)
				{
					InworldController.Error = e.ToString();
				}
			}
			finally
			{
		...
		internal void ResolveGRPCPackets(GrpcPacket response)
		{
